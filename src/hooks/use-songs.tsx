...
export function SongProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadSongs = async () => {
    ...
    try {
      ...
      setSongs(songsData.map(song => ({
        ...
        notes: song.notes || "",
        files: song.files || [],
        keyHistory: song.song_keys?.map(key => ({
          id: key.id,
          key: key.key,
  ...
  const addSong = async (songData: Partial<Song>): Promise<Song> => {
    ...
    try {
      ...
      const newSong: Song = {
        ...
        notes: data.notes || "",
        files: data.files || [],
        keyHistory: data.song_keys?.map(key => ({
          id: key.id,
          key: key.key,
          playedAt: key.played_at,
          setlistId: key.setlist_id,
  ...
  const updateSong = async (id: string, songData: Partial<Song>): Promise<Song> => {
    if (!user) throw new Error("User not authenticated");
  ...
  const deleteSong = async (id: string): Promise<void> => {
    ...
    try {
      ...
      if (songToDelete?.files?.length) {
        const filePaths = songToDelete.files.map(f => f.path);
  ...
  return (
    <SongContext.Provider value={{ songs, addSong, updateSong, deleteSong, isLoading, error }}>
      {children}
