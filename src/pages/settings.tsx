...
export default function Settings() {
  const { settings, updateOneTouchSongs, isLoading: isSettingsLoading } = useSettings();
  ...
  const handleRemoveSong = (songId: string) => {
    const newSongIds = settings.oneTouchSongs.songIds.filter(id => id !== songId);
        ...
        <div className="grid gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Add up to three songs that you can quickly access from anywhere in the app
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isSettingsLoading ? (
                ...
                <>
                  {oneTouchSongs.length === 0 && !isAdding ? (
                    <div className="text-center py-6">
                      <p className="text-muted-foreground mb-4">No One-Touch Songs added yet</p>
                      <Button 
